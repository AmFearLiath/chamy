...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .card { background: linear-gradient(180deg, var(--surface), var(--surface-2)); border: 1px solid var(--border); border-radius: 14px; padding: 18px; margin-bottom: 16px; }
        .grid { display: grid; gap: 12px; grid-template-columns: repeat(2, minmax(0,1fr)); }
        .field { display: flex; flex-direction: column; gap: 6px; }
        .field label { font-size: 13px; font-weight: 600; }
        .field small { color: var(--muted); font-size: 12px; }
        input, select { width: 100%; padding: 10px 12px; border-radius: 10px; border: 1px solid var(--border); background: #0f141b; color: var(--text); }
        .full { grid-column: 1 / -1; }
        .error { color: #ffd7d7; background: rgba(255,107,107,.15); border: 1px solid rgba(255,107,107,.4); border-radius: 10px; padding: 10px; margin-bottom: 10px; }
        .ok { color: #d5ffe3; background: rgba(34,197,94,.15); border: 1px solid rgba(34,197,94,.45); border-radius: 10px; padding: 12px; }
        .btn { background: var(--accent); color: #00142d; border: 0; font-weight: 700; border-radius: 10px; padding: 11px 16px; cursor: pointer; }
...
<div class="wrap">
    ...
    <form method="post" action="/install">
        ...
        <div class="card">
            <h2 class="section-title">1) Projekt & Basis</h2>
            <div class="grid">
                <div class="field">
                    <label>APP_NAME</label>
                    <input name="app_name" value="<?= h($values['app_name']) ?>" required>
                    <small>Min 2, Max 80 Zeichen. Beispiel: <code>Chamy CMS</code></small>
                </div>
                <div class="field">
                    <label>APP_URL</label>
                    <input name="app_url" value="<?= h($values['app_url']) ?>" required>
                    <small>Vollständige URL inkl. Port. Beispiel: <code>http://localhost:8080</code></small>
                </div>
                <div class="field">
                    <label>APP_ENV</label>
                    <select name="app_env">
                    ...
                    <small><code>production</code> für Livebetrieb, <code>development</code> für lokale Entwicklung.</small>
                </div>
                <div class="field">
                    ...
                    <small>In Produktion immer <code>false</code>.</small>
                </div>
                <div class="field">
                    <label>APP_LOCALE</label>
                    <select name="app_locale">
                        <option value="de" <?= $values['app_locale'] === 'de' ? 'selected' : '' ?>>de</option>
                        <option value="en" <?= $values['app_locale'] === 'en' ? 'selected' : '' ?>>en</option>
                    </select>
                </div>
                <div class="field">
                    <label>APP_FALLBACK_LOCALE</label>
                    <select name="app_fallback_locale">
                        <option value="en" <?= $values['app_fallback_locale'] === 'en' ? 'selected' : '' ?>>en</option>
                        <option value="de" <?= $values['app_fallback_locale'] === 'de' ? 'selected' : '' ?>>de</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <h2 class="section-title">2) Datenbank</h2>
            <p class="help">Woher bekomme ich diese Werte? In XAMPP/MAMP/Docker aus der jeweiligen DB-Konfiguration oder aus eurem Hosting-Panel (MySQL Credentials).</p>
            <div class="grid">
                <div class="field">
                    ...
                    <small>Aktuell für den Installer vorgesehen: <code>mysql</code>.</small>
                </div>
                <div class="field">
                    <label>DB_HOST</label>
                    <input name="db_host" value="<?= h($values['db_host']) ?>" required>
                    <small>Beispiel: <code>127.0.0.1</code> oder <code>db</code> (Docker).</small>
                </div>
                <div class="field">
                    <label>DB_PORT</label>
                    <input name="db_port" type="number" min="1" max="65535" value="<?= h($values['db_port']) ?>" required>
                    <small>Min 1, Max 65535, Standard MySQL: <code>3306</code>.</small>
                </div>
                <div class="field">
                    <label>DB_DATABASE</label>
                    <input name="db_database" value="<?= h($values['db_database']) ?>" required>
                    <small>Vorab in MySQL anlegen, z. B. <code>chamy</code>.</small>
                </div>
                <div class="field">
                    <label>DB_USERNAME</label>
                    <input name="db_username" value="<?= h($values['db_username']) ?>" required>
                    <small>DB-Benutzer mit CREATE/ALTER/INSERT/UPDATE/DELETE-Rechten.</small>
                </div>
                <div class="field">
                ...
                <div class="field">
                    ...
                    <input name="session_lifetime" type="number" min="5" max="1440" value="<?= h($values['session_lifetime']) ?>" required>
                    <small>Min 5, Max 1440 (24h), empfohlen: 120.</small>
                </div>
            </div>
        </div>

        <div class="card">
            <h2 class="section-title">3) Admin-Konto</h2>
            <div class="grid">
                <div class="field">
                    <label>Admin Username</label>
                    <input name="admin_username" value="<?= h($values['admin_username']) ?>" required>
                    <small>3-32 Zeichen, erlaubt: a-z, A-Z, 0-9, ., _, -</small>
                </div>
                <div class="field">
